...
### Step 1.1: Design First (Use frontend-design Skill)

**⚠️ ALWAYS start with design planning!**

Activate the `frontend-design` skill to create `design_brief.md`:

```bash
# The frontend-design skill will:
# 1. Analyze the user request
# 2. Plan the UI structure
# 3. Define components and layout
# 4. Create design_brief.md
```
...
### Step 1.4: Prepare App Definitions
...
You can add more apps to an existing dashboard at any time!

When you call `create_apps` again:
- Existing apps are **NOT recreated** (read from app_metadata.json)
- Only NEW apps are created
- Metadata is **merged** (existing + new)
- TypeScript is regenerated with **ALL apps**

**Example: Adding Categories to existing Tasks app**

```
// User: "Ich möchte auch Kategorien für meine Tasks"

// Just call create_apps with the new app:
mcp__dashboard_tools__create_apps({
  "apps": [
    {
      "name": "Categories",
      ...
      "controls": {
        "name": { "fulltype": "string/text", "label": "Name", "required": true },
        "color": { "fulltype": "string/text", "label": "Color" }
      }
    }
  ]
})

// Then regenerate TypeScript with combined metadata
mcp__dashboard_tools__generate_typescript({ "metadata": <returned_metadata> })

// Now update the UI to use the new Categories type
```

The tool automatically:
1. Detects existing apps from `app_metadata.json`
2. Only creates the new "Categories" app
3. Returns metadata with both Tasks AND Categories
4. Saves updated metadata for future use
